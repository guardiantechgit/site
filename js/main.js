...
/* ===================================
 Anti-copy / Anti-selection (soft protection)
 ====================================== */
(function () {
    "use strict";

    // Permite exceções via classe:
    // - .allow-select: permite selecionar/copiar naquela área
    // - .allow-contextmenu: permite botão direito naquela área

    const STYLE_ID = "anti-copy-style";

    // Injeta CSS (mais performático) para bloquear seleção no site todo,
    // mantendo seleção/edição em inputs/textarea/contenteditable e áreas liberadas.
    ...
    const isEditable = (el) => {
        if (!el) return false;
        return (
            el.closest?.("input, textarea, select, option, [contenteditable='true'], .allow-select") != null
        );
    };

    const allowContextMenu = (el) => {
        if (!el) return false;
        return el.closest?.(".allow-contextmenu") != null;
    };

    // Bloqueia botão direito (menu de contexto), exceto em áreas liberadas/inputs
    document.addEventListener(
        "contextmenu",
        (e) => {
            if (isEditable(e.target) || allowContextMenu(e.target)) return;
            e.preventDefault();
        },
        { capture: true }
    );

    // Bloqueia início de seleção (click+arrastar), exceto em áreas liberadas/inputs
    document.addEventListener(
        "selectstart",
        (e) => {
    ...
    // Opcional (recomendado se você quer “não permitir copiar” de fato):
    // bloqueia eventos de copiar/recortar fora de campos editáveis/áreas liberadas
    document.addEventListener(
        "copy",
        (e) => {
            if (isEditable(e.target)) return;
    ...
    // Evita arrastar imagens/texto para “copiar”
    document.addEventListener(
        "dragstart",
        (e) => {
            if (isEditable(e.target)) return;
   ...
   /* ===================================
     ...
     ====================================== */
    (function($) {
        // seleciona o input de telefone e marca como obrigatório
        ...
        // aplica a máscara enquanto digita e ao perder foco
        $phone.on('input blur', maskPhone);
    ...
    if ($.fn.countdown !== undefined && $.fn.countdown !== null) {
        $('.countdown').each(function () {
            ...
            _this.countdown(_this.attr("data-enddate")).on('update.countdown', function (event) {
                _this.html(event.strftime('' + '<div class="counter-container"><div class="countdown-box first"><div class="number">%-D</div><span>Dias</span></div>' + '<div class="countdown-box"><div class="number">%H</div><span>Horas</span></div>' + '<div class="countdown-box"><div class="number">%M</div><span>Minutos</span></div>' + '<div class="countdown-box last"><div class="number">%S</div><span>Segundos</span></div></div>'));
            }).on('finish.countdown', function (event) {
                _this.html(event.strftime('' + '<div class="counter-container"><div class="countdown-box first" data-number="00"><div class="number">00</div><span>Dias</span></div>' + '<div class="countdown-box"><div class="number">00</div><span>Horas</span></div>' + '<div class="countdown-box"><div class="number">00</div><span>Minutos</span></div>' + '<div class="countdown-box last"><div class="number">00</div><span>Segundos</span></div></div>'));
            });
        });
    }
...
$(document).on('submit', '.contact-form-style-01 form', function(e) {
    e.preventDefault();

    var formObj     = $(this),
        _this       = formObj.find('.submit'),
        emailRe     = /^([\w-\.]+@([\w-]+\.)+[\w-]{2,4})?$/,
        urlRe       = /(ftp|http|https):\/\/(\S+)/,
        telRe       = /^[0-9\-\(\)\s\+]+$/,
    ...
    // limpa estados (remove is-invalid e is-valid antigos)
    formObj.find('.required, .terms-condition').removeClass('is-invalid is-valid');
    ...
    formObj.find('.required').each(function() {
        ...
        if (!v) {
            error = true;
            return $f.addClass('is-invalid');
        }

        // input[type="text"] (nome) ≥ 3 caracteres
        if ($f.is('input[type="text"]') && v.length < 3) {
            error = true;
            return $f.addClass('is-invalid');
        }

        // email: formato + ≥10 caracteres
        if ($f.is('[type="email"]')) {
            if (!emailRe.test(v) || v.length < 10) {
                error = true;
                return $f.addClass('is-invalid');
            }
            // válido
            return $f.addClass('is-valid');
        }

        // telefone: formato + 10–11 dígitos numéricos
        if ($f.is('[type="tel"]')) {
            var digits = v.replace(/\D/g, '');
            if (!telRe.test(v) || digits.length < 10 || digits.length > 11) {
                error = true;
                return $f.addClass('is-invalid');
            }
            // válido
        ...
        // textarea (mensagem) ≥ 15 caracteres
        if ($f.is('textarea')) {
            if (v.length < 15) {
                error = true;
                return $f.addClass('is-invalid');
        ...
        // qualquer outro campo (por via das dúvidas)
        $f.addClass('is-valid');
    });

    // valida termos
    var terms = formObj.find('.terms-condition');
    if (terms.length && !terms.is(':checked')) {
        error = true;
        terms.addClass('is-invalid');
    }

    if (error || !actionURL) return;
    ...
    $.ajax({
        ...
        success: function(result) {
            ...
            // redireciona se precisar
            if (redirectVal) {
                return window.location.href = redirectVal;
            }

            // parseia JSON se for string
            if (typeof result === 'string') {
                result = $.parseJSON(result);
            }

            // limpa inputs e validações
            formObj
              .find('input[type=text], input[type=url], input[type=email], input[type=tel], input[type=password], textarea')
              .val('').removeClass('is-invalid is-valid');
        ...
        error: function() {
            ...
            resultsObj
              ...
              .html('Ocorreu um erro, tente novamente mais tarde.')
              .fadeIn('slow')
              .delay(4000)
              .fadeOut('slow');
        }
...
// dispara o submit quando clicar no botão
